...
- [x] **6.5** Web Store packaging + privacy policy doc (`PRIVACY.md` listing exactly
  ...
  **Playwright:** built zip passes the Web Store manifest linter.
  `scripts/package.mjs` (build → `out/negative-filter-<v>.zip` via
...
2026-05-25 · 6.5 · scripts/package.mjs (npm run package) → out/negative-filter-<v>.zip via system zip. PRIVACY.md enumerates storage keys (nf:llm-settings, nf:filters:*, nf:spend-ledger, nf:deep-warning-dismissed, nf:deep-queue) + the two outbound destinations (LLM provider + same-origin detail tabs). Spec: unzip + assert mv3 + required string fields + KNOWN_PERMISSIONS membership + no host_permissions baked + referenced paths resolve; PRIVACY.md mentions every key.
